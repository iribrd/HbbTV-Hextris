...
function showHelp() {
	...
	$("#inst_main_body").html("<div id = 'instructions_head'>چطور بازی کنید</div><p>.هدف بازی جلوگیری از خروج رنگها از شش ضلعی می باشد</p><p>" + (settings.platform != 'mobile' ? 'دکمه چپ و راست در کلیدهای جهت نما را فشار دهید' : 'به سمت چپ و راست صفحه نمایش ضربه بزنید') + " تا شش ضلعی بچرخد</p><p>با رسیدن سه خانه و بیشتر رنگ های مشابه پاک می شوند و امتیاز میگیرید</p><p>زمان باقیمانده با کاهش خط های باریک اطراف نمایش داده می شود <span style='color:#f1c40f;'>خط های</span> <span style='color:#e74c3c'>رنگی</span> <span style='color:#3498db'>نباید</span> <span style='color:#2ecc71'>از</span>شش ضلعی خارج شوند</p> <hr> <p id = 'afterhr'></p>");
	if (gameState == 1) {
		pause();
	}
